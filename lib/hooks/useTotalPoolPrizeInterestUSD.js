...
export const useTotalPoolPrizeInterestUSD = () => {
  ...
  const { data: prizePools } = useMultiversionPoolsLockedValue()
  ...
  prizePools &&
    Object.keys(prizePools).forEach((key) => {
      prizePoolAddresses.push(prizePools[key].id)
    })
  ...
  try {
    ...
    prizePools.forEach((prizePool) => {
      ...
      batchCalls.push(
        ...
        prizeStrategyContract.prizePeriodRemainingSeconds(),
    ...
    prizePools.forEach((prizePool) => {
      ...
      const prizePeriodRemainingSeconds =
        results[prizeStrategyAddress].prizePeriodRemainingSeconds[0]
